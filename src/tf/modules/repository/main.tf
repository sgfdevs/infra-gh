...
resource "github_repository_ruleset" "main" {
  ...
  rules {
    ...
    pull_request {
      ...
      required_approving_review_count   = length(var.review_bypass_integrations) == 0 ? var.required_approving_review_count : 0
...
resource "github_repository_ruleset" "required_reviews" {
  count = length(var.review_bypass_integrations) == 0 ? 0 : 1
